...
6. **Strategy** — две стратегии, взаимоисключающие по режиму (никогда не
   ...
   * **Сетка** (`strategy/grid_strategy.py`): включается только в `range`;
     ...
     известного тренда.
   * **Тренд / momentum** (`strategy/trend_strategy.py`): включается
     только в `uptrend`/`downtrend` — пробойный вход (агрессивная лимитка,
     пересекающая спред, чтобы исполниться сразу), но ТОЛЬКО если движение
     подтверждено объёмом (`features.volume_vs_avg >= min_volume_ratio`,
     DEFAULT 1.3x среднего) — без этого гораздо чаще ловим ложные пробои;
     без усреднения (пока позиция открыта, новых входов нет); выход по
     фиксированному % стоп-лосс/тейк-профит от цены входа
     (`strategy_trend.*` в конфиге).

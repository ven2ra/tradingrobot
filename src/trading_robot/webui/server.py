...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .badge-update       { background: rgba(191,143,255,.15); color: #bf8fff; }
  ...
  <div class="wrap">
    <table>
      <thead><tr>
        ...
        <th>Что сделал робот<span class="hint" title="enter — выставил заявку, skip — ничего не сделал (пропустил такт), cancel — отменил заявку, flatten — аварийно закрыл позицию, sync — сверил состояние с брокером при старте, update — статус ранее выставленной заявки изменился (исполнена/отменена/частично исполнена) — это отдельная запись, появляется ПОЗЖЕ строки с enter по той же заявке, когда её судьба на бирже стала известна.">?</span></th>
        <th>Причина</th>
        <th>Цена</th>
...
const ACTION_RU = {
  ...
  update: 'Статус заявки изменился',
};
...
const REASON_RULES = [
  ...
  [/^order status changed to (\w+)$/, m => `Статус заявки на бирже обновился: ${STATUS_RU[m[1]] || m[1]}`],
  [/^order no longer active: inferred filled.*$/, () => 'Заявка пропала из активных на бирже, а позиция изменилась в её сторону — вывод: скорее всего исполнена'],
